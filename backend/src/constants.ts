...
// Prompts
export const PROMPT_COLORING_BOOK = `Narysuj czarno-białą ilustrację do kolorowania (bez tła, brak szarości, brak cieniowania, brak czarnych wypełnień np. bluzek, bez ramek, obrazek binanry - tylko czerń i biel). Nie dodawaj żadnego tekstu na obrazku, chyba ze jest to wprost powiedziane (to, że chce aby np. wygenerować Wiktora to nie znaczy, że chce napis Wiktor). Jeeli podałem jakieś referencje (inlineData) to niech one będą szczegółowe i wiernie oddawala wyslane pliki jezeli chodzi o twarze. Temat: `;
